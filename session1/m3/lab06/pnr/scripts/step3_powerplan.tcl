...
open_lib $ARC_TOP
copy_block -from_block 04_${DESIGN_NAME}_floorplan_placed -to 05_${DESIGN_NAME}_floorplan_pns_start -force
open_block 05_${DESIGN_NAME}_floorplan_pns_start
################################################################################Checking
report_lib $ARC_TOP
################################################################################Power_Planing
...
create_pg_mesh_pattern TOP_MESH_VERTICAL \
	-layers " \
		{ {vertical_layer: M6}   {width: 0.2} {spacing: interleaving} {pitch: 4} {offset: 0.5}  {trim : true} } \
		" 
...
create_pg_mesh_pattern TOP_MESH_HORIZONTAL \
	-layers " \
		{ {horizontal_layer: M7}   {width: 0.2} {spacing: interleaving} {pitch: 4} {offset: 0.5}  {trim : true} } \
		" 
...
################################################################################Save_Block
save_block -as 06_${DESIGN_NAME}_floorplan_pns_finish
save_lib
close_block
close_lib
